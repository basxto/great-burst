    ...
    if(offset > 0){
        ...
        for(i = 5; i > 0; --i){//change column
            for(j = 0; j < 6; ++j){//go through column
                background[ (j+4) *20 + i + offset] = background[ (j+4) *20 + i];
...
void splash(){
    ...
    //1-2
    for(i = 2; i <= 3; ++i){
        set_sprite_tile((i+6),oga_splash_movable_map[ splashscreen_a_xoffset+i ]);
        move_sprite((i+6),i<<3,0);
    }
    //3-6
    for(i = 1; i <= 4; ++i){
        set_sprite_tile((i+9),oga_splash_movable_map[ splashscreen_line_width + splashscreen_a_xoffset+i ]);
        move_sprite((i+9),i<<3,1<<3);
    }
    //7-10
    for(i = 1; i <= 4; ++i){
        set_sprite_tile((i+13),oga_splash_movable_map[ splashscreen_line_width*2 + splashscreen_a_xoffset+i ]);
        move_sprite((i+13),i<<3,2<<3);
    }
    //11-16
    for(i = 0; i <= 5; ++i){
        set_sprite_tile((i+18),oga_splash_movable_map[ splashscreen_line_width*3 + splashscreen_a_xoffset+i ]);
        move_sprite((i+18),i<<3,3<<3);
    }

    //17-22
    for(i = 0; i <= 5; ++i){
        set_sprite_tile((i+24),oga_splash_movable_map[ splashscreen_line_width*4 + splashscreen_a_xoffset+i ]);
        move_sprite((i+24),i<<3,4<<3);
    }

    //23-24 at index 13 and 14
    set_sprite_tile(30,oga_splash_movable_map[ splashscreen_line_width*5 + splashscreen_a_xoffset ]);
    move_sprite(30,0,5<<3);
    set_sprite_tile(31,oga_splash_movable_map[ splashscreen_line_width*5 + splashscreen_a_xoffset+5 ]);
